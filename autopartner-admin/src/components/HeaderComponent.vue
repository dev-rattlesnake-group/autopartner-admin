...
<template>
  <div class="header-wrapper">
    ...
    <div class="header-main">
      <div class="header-main-up" v-show="props.screenWidth > 850">
        ...
        <div class="header-main-up_control font-semibold" v-if="crumbs.length">
          <div v-if="authStore.user?.role == 'admin'" class="rounded-md border-[1px] py-[0.3rem] px-[1rem] leading-normal">Main Admin</div>
          <div @click="router.push({name:'shopping-cart'})" v-if="authStore.user?.role == 'customer'" class="cursor-pointer flex items-center gap-2 text-md rounded-md py-[0.3rem] px-[1rem] leading-normal bg-[#C40F30] text-white"> 
          ...
          <NotificationIcon />
          <button class="header-main-up_control_button"><UsersIcon /></button>
        </div>
      ...
      <div
        class="header-main-down h-[1.8rem] flex items-center"
      ...
      >
        <div class="header-main-down-breadcrumbs">
          ...
          <span> / </span>
          <span
            class="header-main-down-breadcrumbs_current"
            @click="router.push({ name: crumbs[0].route })"
            >{{ crumbs[0].name }}</span
          >
          <span v-if="crumbs.length > 1">
            ...
            <span
              class="header-main-down-breadcrumbs_current"
              @click="router.push({ name: crumbs[1].route })"
              >{{ crumbs[1].name }}</span
            >
          </span>
        </div>
    ...
    <div class="flex w-full justify-end h-full items-center pr-6" v-if="props.screenWidth < 850 && router.currentRoute.value.name!=='login'"> 
