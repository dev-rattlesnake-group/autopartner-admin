...
onMounted(async () => {
  emit('header-data', {
    title: 'Favorites',
    crumbs: [{ name: 'favorites', route: 'favorites' }]
  })
  await productStore.getFavorites()
  // if(!productStore.favoriteItemProductIds.length) return
  setTimeout(async () => {
    const params = {
      filter: {
        ...state.filters,
        id: productStore.favoriteItemProductIds?.length ? productStore.favoriteItemProductIds : [-1]
      },
      page: state.page,
      size: state.size,
      sort: state.sort
...
watch(
  ...
  async (val) => {
    const params = {
      filter: { ...state.filters, id: productStore.favoriteItemProductIds.length ? productStore.favoriteItemProductIds : [-1] },
      page: state.page,
      size: state.size,
      sort: state.sort
    }
...
<template>
  <div class="w-full max-w-screen h-full flex-start text-black p-4 flex-col oveflow-y-auto overflow-x-hidden gap-4 ">
