...
onMounted(async () => {
  emit('header-data', {
    ...
    crumbs: [
      ...
      { name: 'Edit Events', route: 'events' }
    ]
  })

  await productStore.getOne(Number(router?.currentRoute?.value?.params?.id))
  Object.keys(productStore.event).forEach(formProperty => {
    switch (formProperty) {
      case 'event_details': {
        if (productStore.event.event_details?.length) {
          productStore.event.event_details?.forEach((det, index) => {
            detailsForm[`value_${index + 1}`] = det
          })
        }
      ...
      case 'image_url': {
        if (productStore.event?.[formProperty]) {
          productForm[formProperty] = productStore.event?.[formProperty]
          const [, fileName] = productStore.event?.[formProperty].split('upload/')
          fileList.value.push({ uuid: 1, name: fileName, url: productStore.event?.[formProperty] })
          break
        }
      ...
      // case 
      default: productForm[formProperty] = productStore.event?.[formProperty]
...
const updateProduct = async () => {
  try {
    ...
    await productStore.update({ event: productForm, event_details })
...
<template>
  <div
    ...
    <div class="md:w-[60%] w-full rounded-lg bg-white h-full flex flex-col items-start p-6 gap-4">
      <p class="title">Редактировать акцию</p>
  ...
  <p class="leading-[100%] text-gray-400 text-sm">Дата истечения акции</p>
            <a-date-picker v-model:value="productForm.expired_date" valueFormat="YYYY-MM-DD" />
          </div>
