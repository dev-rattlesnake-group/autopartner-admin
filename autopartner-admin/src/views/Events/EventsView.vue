...
const fetchProducts = async () => {
  ...
  try {
    ...
    columns.value.forEach((column) => {
      let filters = productStore.events.map((product) => {
...
<template>
  <div class="customers-wrapper">
    ...
    <div class="customer-table">
      <a-table :columns="responsiveColumns" :loading="isLoading" :data-source="productStore.events" @change="onChange"
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column?.key === 'event_details'">
            <div class="text-sm">
              <p v-for="ev, ind in record.event_details" :key="ind" class="text-xs md:text-sm"> {{ ev?.title }}</p>
            </div>
          </template>
          ...
          <template v-if="column?.key === 'title'">
            <div class="cursor-pointer flex items-center gap-4"
              @click="router.push({ name: 'updateEvent', params: { id: record.id } })">
              <div
