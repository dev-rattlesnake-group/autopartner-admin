...
import { useVacancyStore, type IVacancy } from '@/stores/vacancy.store'
...
const fetchProducts = async () => {
  ...
  try {
    await productStore.get({ page: state.page, take: state.size, sort: state.sort })
    columns.value.forEach((column) => {
      let filters = productStore.vacancies.map((product: IVacancy): string => {
        // @ts-ignore
...
<template>
  <div class="customers-wrapper">
    ...
    <div class="customer-table">
      <a-table :columns="responsiveColumns" :loading="isLoading" :data-source="productStore.vacancies"
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column?.key === 'title'">
            <div class="cursor-pointer flex items-center gap-4"
              @click="router.push({ name: 'updateVacancy', params: { id: record.id } })">
              <!-- <div
