...
onMounted(async () => {
  ...
  await productStore.getNew(Number(router?.currentRoute?.value?.params?.id))
  Object.keys(productForm).forEach(formProperty => {
    switch (formProperty) {
      ...
      // case 
      default: productForm[formProperty] = productStore.new?.[formProperty]
...
const updateProduct = async () => {
  try {
    ...
    await productStore.updateNews(productForm)
...
const deleteProduct = async () => {
  await productStore.deleteNews(productForm)
  router.push({ name: 'news' })
}
