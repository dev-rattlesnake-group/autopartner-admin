...
const fetchProducts = async () => {
  ...
  try {
    await productStore.getNews({ page: state.page, take: state.size, sort: state.sort })
    columns.value.forEach((column) => {
      let filters = productStore.news.map((product) => {
...
const onChange = async (
  ...
  try {
    ...
    await productStore.getNews(params)
  } catch (e) {
    console.log(e)
    message.error(e.response?.data?.message || e.message || JSON.stringify(e))
  }
...
<template>
  <div class="customers-wrapper">
    ...
    <div class="customer-table">
      <a-table :columns="responsiveColumns" :loading="isLoading" :data-source="productStore.news" @change="onChange"
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column?.key === 'title'">
            <div class="cursor-pointer flex items-center gap-4"
              @click="router.push({ name: 'updateNews', params: { id: record.id } })">
              <div
